...
public class DataDogDestination: BaseDestination {
    ...
    public init(dataDogLogDelegate: DataDogLogDelegate, owner: XCGLogger, formatter: LogFormatterProtocol? = nil, outputLevel: XCGLogger.Level = .info, identifier: String = "") throws {
        if !Datadog.isInitialized() {
            throw NSError(domain: "DataDogDestination", code: 0, userInfo: [NSLocalizedDescriptionKey: "Datadog is not initialized"])
        }
        self.dataDogLogDelegate = dataDogLogDelegate
    ...
    public override func output(logDetails: LogDetails, message: String) {
        ...
        self.write(level: logDetails.level, tag: logDetails.userInfo[_Key.tagKey] as? String ?? "", message: message, date: logDetails.date, error: logDetails.userInfo[_Key.errorKey] as? Error, callStackSymbols: logDetails.userInfo[_Key.stackTraceKey] as? String)
    }
